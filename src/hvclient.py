...
class HvClient(object):
    ...
    def newClient(self):
        if self.configpath != None:
            ...
            logging.info('File not found in ' + self.configpath + ' checking the environment variables now')
            try:
                ...
                logging.info('Environment variables needed to connect to cluster are fetched')
            except KeyError:
                logging.error('Error encountered ' + str(KeyError))
                sys.exc_clear()

        logging.info('Client cert ' + client_cert)
        ...
        try:
            if client.sys_is_initialized() and client.is_authenticated():
                return client
